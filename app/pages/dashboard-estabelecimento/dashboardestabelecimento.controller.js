 ...
 * Modified by Saymon on 25/01/17. ** Indenta o código e Corrige datas do filtro de atendimentos e vendedores **
 * Modified by Saymon on 21/02/17.
 */
angular.module("teewa").controller("dashboardEstabelecimentoCtrl", function ($filter, $scope, $http, config, $state, $stateParams, sharedConn, Chats, ChatDetails) {
    ...
    $scope.denunciasPorDia = function (date_start, date_end) {
        $http({
            url: config.baseUrl + "/dash/complaints/by/date",
            method: 'post',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': config.token
            ...
            $scope.dadosDenunciaDia = data;
            console.log('DENUNCIA POR DIAA!!')
            console.log($scope.dadosDenunciaDia);
            //graficoDenunciaPorDia(dado);
        }).error(function (error) {
            $scope.message = "Aconteceu um problema: " + error;
        });
    };
    ...
    function graficoDenunciaPorDia(dado) {
        ...
        $scope.denunciasPorDia("01/01/2015","24/12/2019");
